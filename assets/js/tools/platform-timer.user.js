...
/*
  這支跟 skhpsv2/assets/js/tools/platform-timer.bookmarklet.js 是同一份核心
...
(function () {
  ...
  // 偵測「這頁像不像院內系統」：有全域 ko(Knockout) 就浮一顆啟動鈕。
  // 院內系統是 SPA，資料可能晚點才載入，所以輪詢一段時間等 ko 出現。
  var LAUNCH_BTN_ID = "skh-platform-timer-launch-btn";
  var tries = 0;

  function addLaunchButton() {
    if (document.getElementById(LAUNCH_BTN_ID)) return;
    var btn = document.createElement("button");
    btn.id = LAUNCH_BTN_ID;
    btn.type = "button";
    ...
    btn.addEventListener("click", function () {
      try { launch(); } catch (e) { alert("啟動失敗：" + (e && e.message ? e.message : e)); }
    });
  ...
  function poll() {
    tries += 1;
    if (typeof window.ko !== "undefined" && window.ko && typeof window.ko.dataFor === "function") {
      addLaunchButton();
      return;
    }
    if (tries < 40) {
